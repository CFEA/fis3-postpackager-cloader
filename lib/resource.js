...
Resource.prototype.buildConf = function(type) {
  ...
  if (/comtop.c5.core\.js|cmod/i.test(type)) {
    return this.buildCMODPath();
  } else if (/sea\.js|cmd/i.test(type)) {
    return this.buildCMDPath();
  } else if (/mod|commonJs/i.test(type)) {
...
Resource.prototype.buildCMODPath = function() {
  var paths = {};
  var self = this;
  ...
  // {paths:' + JSON.stringify(paths, null, 2) + '});
  return 'C5.config({alias:' + JSON.stringify(paths, null, self.resourcemapWhitespace) + '});';
};
